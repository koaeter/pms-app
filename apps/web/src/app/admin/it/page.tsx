...
type Audit = { id: string; userId: string | null; action: string; module: string; entityType: string; entityId: string | null; severity: string; createdAt: string; user: { username: string } | null };
...
export default function ItAdminPage() {
  ...
  const [tab, setTab] = useState<'users' | 'sessions' | 'security' | 'audit'>('users');
  ...
  async function load() {
    ...
    try {
      const [h, u, s, e, a] = await Promise.all([get('/admin/it/health'), get('/admin/it/users'), get('/admin/it/sessions'), get('/admin/it/authentication-events'), get('/admin/it/audit-logs')]);
      setHealth(h); setUsers(u); setSessions(s); setEvents(e); setAudits(a);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <div className="content">
        ...
        <section className="panel"><div className="tab-bar"><button className={tab === 'users' ? 'selected' : ''} onClick={() => setTab('users')}>Users</button><button className={tab === 'sessions' ? 'selected' : ''} onClick={() => setTab('sessions')}>Sessions</button><button className={tab === 'security' ? 'selected' : ''} onClick={() => setTab('security')}>Security events</button><button className={tab === 'audit' ? 'selected' : ''} onClick={() => setTab('audit')}>Audit log</button></div>
          {loading ? <p className="muted">Loading administration data…</p> : tab === 'users' ? <div className="admin-table"><div className="admin-row admin-head"><span>Account</span><span>Employee</span><span>Role</span><span>Status</span><span>Actions</span></div>{users.map(u => <div className="admin-row" key={u.id}><span><strong>{u.username}</strong><small>{u.mfaEnabled ? 'MFA enabled' : 'MFA not enabled'}</small></span><span>{u.employee.firstName} {u.employee.lastName}<small>{u.employee.employeeNumber}</small></span><span>{u.roles.map(r => r.role.name).join(', ') || 'No role'}</span><span><span className="status">{u.accountStatus.replaceAll('_', ' ')}</span></span><span className="action-buttons"><button onClick={() => revoke(u.id)}>Revoke sessions</button>{u.accountStatus === 'ACTIVE' ? <button onClick={() => setStatus(u.id, 'SUSPENDED')}>Suspend</button> : <button onClick={() => setStatus(u.id, 'ACTIVE')}>Activate</button>}</span></div>)}</div> : tab === 'sessions' ? <div className="admin-table"><div className="admin-row admin-head"><span>User</span><span>IP</span><span>Last activity</span><span>Status</span><span>User agent</span></div>{sessions.map(s => <div className="admin-row" key={s.id}><span>{s.user.username}</span><span>{s.ipAddress ?? '—'}</span><span>{new Date(s.lastActivityAt).toLocaleString()}</span><span>{s.revokedAt ? 'Revoked' : 'Active'}</span><span>{s.userAgent ?? '—'}</span></div>)}</div> : tab === 'security' ? <div className="admin-table"><div className="admin-row admin-head"><span>User</span><span>Event</span><span>Result</span><span>Time</span></div>{events.map(e => <div className="admin-row" key={e.id}><span>{e.user.username}</span><span>{e.eventType.replaceAll('_', ' ')}</span><span>{e.success ? 'Success' : 'Failed'}</span><span>{new Date(e.createdAt).toLocaleString()}</span></div>)}</div> : <div className="admin-table"><div className="admin-row admin-head"><span>Actor</span><span>Action</span><span>Module</span><span>Entity</span><span>Severity</span><span>Time</span></div>{audits.map(a => <div className="admin-row" key={a.id}><span>{a.user?.username ?? 'System'}</span><span>{a.action.replaceAll('_', ' ')}</span><span>{a.module}</span><span>{a.entityType}<small>{a.entityId ?? '—'}</small></span><span>{a.severity}</span><span>{new Date(a.createdAt).toLocaleString()}</span></div>)}</div>}
        </section>
      </div>
    </section>
  </main>;
