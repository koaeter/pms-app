...
export class PerformanceService {
  ...
  async createCycle(organizationId: string, programId: string, dto: CreateCycleDto) {
    ...
    if (!Number.isFinite(startDate.getTime()) || !Number.isFinite(endDate.getTime())) {
      throw new ConflictException('Cycle dates must be valid dates');
    }
    if (endDate <= startDate) {
      throw new ConflictException('Cycle end date must be after cycle start date');
    }
    try {
  ...
  listKpis(organizationId: string) {
    ...
    try {
      return await this.prisma.kpi.create({
        data: {
          ...
          code: dto.code.trim().toUpperCase(),
          description: dto.description?.trim() || null,
          measurementMethod: dto.measurementMethod?.trim() || null,
  ...
  listCompetencies(organizationId: string) {
  ...
  async createCompetency(organizationId: string, dto: CreateCompetencyDto) {
    try {
      ...
      if ((error as { code?: string }).code === 'P2002') {
        throw new ConflictException('Competency code already exists');
      }
      throw error;
    }
  }

  listRatingScales(organizationId: string) {
    return this.prisma.ratingScale.findMany({
      where: { organizationId, active: true },
  ...
  async createRatingScale(organizationId: string, dto: CreateRatingScaleDto) {
    if (!dto.items?.length) {
      throw new ConflictException('A rating scale must contain at least one item');
    }

    const values = dto.items.map((item) => item.value);
    if (new Set(values).size !== values.length) {
      throw new ConflictException('Rating scale values must be unique');
    }

    for (const item of dto.items) {
      if (!Number.isFinite(item.value)) {
        throw new ConflictException('Rating scale values must be finite numbers');
      }
      if (item.minScore !== null && item.minScore !== undefined && !Number.isFinite(item.minScore)) {
        throw new ConflictException('Rating scale minimum scores must be finite numbers');
      }
      if (item.maxScore !== null && item.maxScore !== undefined && !Number.isFinite(item.maxScore)) {
        throw new ConflictException('Rating scale maximum scores must be finite numbers');
      }
      if (item.minScore !== null && item.minScore !== undefined && item.maxScore !== null && item.maxScore !== undefined && item.maxScore < item.minScore) {
        throw new ConflictException('Rating scale maximum score cannot be below its minimum score');
      }
    }

    try {
      return await this.prisma.ratingScale.create({
        data: {
          organizationId,
          name: dto.name.trim(),
          code: dto.code.trim().toUpperCase(),
      ...
      if ((error as { code?: string }).code === 'P2002') {
        throw new ConflictException('Rating scale code or value already exists');
      }
      throw error;
    }
  }
  ...
  private reviewInclude() {
    return {
      employee: true,
      performanceCycle: { include: { program: true } },
      reviewType: true,
  ...
  async listMyReviews(organizationId: string, employeeId: string) {
    return this.listReviews(organizationId, employeeId);
  }

  async listTeamReviews(organizationId: string, supervisorEmployeeId: string, isHod: boolean) {
    if (!isHod) {
      return this.prisma.performanceReview.findMany({
        where: { employee: { organizationId }, supervisorEmployeeIdSnapshot: supervisorEmployeeId },
        orderBy: { createdAt: 'desc' },
        include: this.reviewInclude(),
      });
    }
    ...
    const assignments = await this.prisma.employeeOrganizationalUnit.findMany({
      where: { supervisorEmployeeId, isPrimary: true, endDate: null },
      select: { organizationalUnitId: true },
    });
    const unitIds = new Set(assignments.map((a) => a.organizationalUnitId));
  ...
  async createReview(organizationId: string, dto: CreateReviewDto) {
    const [employee, cycle, reviewType] = await Promise.all([
      ...
      this.prisma.performanceCycle.findFirst({
        where: {
          id: dto.performanceCycleId,
          program: { organizationId },
          status: { in: ['DRAFT', 'OPEN'] },
        },
        include: { program: true },
      }),
      this.prisma.performanceReviewType.findFirst({
        where: { id: dto.reviewTypeId, active: true, program: { organizationId } },
      }),
    ]);
    ...
    if (cycle.programId !== reviewType.programId) {
      throw new ConflictException('Review type and cycle must belong to the same program');
    }

    const assignment = await this.prisma.employeeOrganizationalUnit.findFirst({
      where: { employeeId: employee.id, isPrimary: true, endDate: null },
      include: { organizationalUnit: true, designation: true, supervisor: true },
    });

    const kpiIds = dto.kpis?.map((item) => item.kpiId) ?? [];
    const competencyIds = dto.competencies?.map((item) => item.competencyId) ?? [];

    if (!kpiIds.length && !competencyIds.length) {
      throw new ConflictException('A review must contain at least one KPI or competency');
    }
    if (new Set(kpiIds).size !== kpiIds.length) {
      throw new ConflictException('A KPI cannot be assigned more than once to a review');
    }
    if (new Set(competencyIds).size !== competencyIds.length) {
      throw new ConflictException('A competency cannot be assigned more than once to a review');
    }

    const allWeights = [
      ...(dto.kpis ?? []).map((item) => Number(item.weight)),
      ...(dto.competencies ?? []).map((item) => Number(item.weight)),
    ];
    if (allWeights.some((weight) => !Number.isFinite(weight) || weight <= 0)) {
      throw new ConflictException('All review assessment weights must be greater than zero');
    }
    if (allWeights.reduce((sum, weight) => sum + weight, 0) <= 0) {
      throw new ConflictException('Review assessment weights must total more than zero');
    }
    ...
    if (kpis.length !== kpiIds.length) {
      throw new NotFoundException('One or more KPIs were not found in the employee organization');
    }
    if (competencies.length !== competencyIds.length) {
      throw new NotFoundException('One or more competencies were not found in the employee organization');
    }
    ...
    try {
          ...
          kpis: {
            create: (dto.kpis ?? []).map((item) => {
              ...
              return {
                kpiId: kpi.id,
                employeeId: employee.id,
                title: item.title?.trim() || kpi.name,
                description: item.description?.trim() || kpi.description,
                target: item.target?.trim() || null,
                measurementUnit: item.measurementUnit?.trim() || kpi.defaultUnit,
                weight: item.weight,
              };
            }),
          },
          competencies: {
            create: (dto.competencies ?? []).map((item) => ({
      ...
      if ((error as { code?: string }).code === 'P2002') {
        throw new ConflictException('A review already exists for this employee, cycle and review type');
      }
      throw error;
    }
  }
  ...
  async scoreKpi(
    ...
    return this.prisma.performanceKpi.update({
      where: { id: kpi.id },
      data:
        actor === 'employee'
          ? { employeeScore: dto.score, actualResult: dto.actualResult?.trim(), employeeComment: dto.comment?.trim() }
          : { supervisorScore: dto.score, actualResult: dto.actualResult?.trim(), supervisorComment: dto.comment?.trim() },
    });
  }
  ...
  async rateCompetency(
    ...
    return this.prisma.performanceCompetency.update({
      where: { id: competency.id },
      data:
        actor === 'employee'
          ? { employeeRating: dto.rating, employeeComment: dto.comment?.trim() }
          : { supervisorRating: dto.rating, supervisorComment: dto.comment?.trim() },
    });
  }
  ...
  async calculateScore(organizationId: string, reviewId: string, dto: CalculateScoreDto) {
    ...
    const review = await this.prisma.performanceReview.findFirst({
      where: { id: reviewId },
      include: { kpis: true, competencies: true },
    });
    if (!review) throw new NotFoundException('Performance review not found');

    if (['FINALIZED', 'LOCKED', 'CANCELLED'].includes(review.status)) {
      throw new ConflictException('Finalized, locked or cancelled reviews cannot be scored');
    }
    if (!review.kpis.length && !review.competencies.length) {
      throw new ConflictException('A review must contain at least one KPI or competency');
    }

    const invalidKpiWeights = review.kpis.some((k) => !Number.isFinite(Number(k.weight)) || Number(k.weight) <= 0);
    const invalidCompetencyWeights = review.competencies.some((c) => !Number.isFinite(Number(c.weight)) || Number(c.weight) <= 0);
    if (invalidKpiWeights || invalidCompetencyWeights) {
      throw new ConflictException('All review assessment weights must be greater than zero');
    }

    const incompleteKpis = review.kpis.filter((k) => k.employeeScore === null && k.supervisorScore === null);
    const incompleteCompetencies = review.competencies.filter((c) => c.employeeRating === null && c.supervisorRating === null);
    if (incompleteKpis.length || incompleteCompetencies.length) {
    ...
    const kpiWeight = review.kpis.reduce((sum, k) => sum + Number(k.weight), 0);
    const competencyWeight = review.competencies.reduce((sum, c) => sum + Number(c.weight), 0);
    const totalWeight = kpiWeight + competencyWeight;
    if (!Number.isFinite(totalWeight) || totalWeight <= 0) {
      throw new ConflictException('Review assessment weights must total more than zero');
    }

    const kpiScore = kpiWeight
      ? review.kpis.reduce((sum, k) => sum + Number(k.supervisorScore ?? k.employeeScore) * Number(k.weight), 0) / kpiWeight
      : null;
    const competencyScore = competencyWeight
      ? review.competencies.reduce((sum, c) => sum + Number(c.supervisorRating ?? c.employeeRating) * Number(c.weight), 0) / competencyWeight
      : null;
    const overallScore =
      ((kpiScore ?? 0) * kpiWeight + (competencyScore ?? 0) * competencyWeight) / totalWeight;

    let overallRating: string | null = null;
    if (dto.ratingScaleId) {
      const scale = await this.prisma.ratingScale.findFirst({
        where: { id: dto.ratingScaleId, organizationId, active: true },
        include: { items: { orderBy: { minScore: 'desc' } } },
      });
      if (!scale) throw new NotFoundException('Rating scale not found');

      const item = scale.items.find(
        (entry) =>
          entry.minScore !== null &&
          overallScore >= Number(entry.minScore) &&
          (entry.maxScore === null || overallScore <= Number(entry.maxScore)),
      );
