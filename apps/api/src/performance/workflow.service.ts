...
export class WorkflowService {
  ...
  async createWorkflow(organizationId: string, dto: CreateWorkflowDto) {
    ...
    if (new Set(orders).size !== orders.length || Math.min(...orders) !== 1 || !orders.every((o, i) => o === i + 1)) throw new ConflictException('Workflow steps must be numbered consecutively from 1');
    if (dto.steps.some((s) => s.actorType === WorkflowActorType.ROLE && !s.actorRoleId)) throw new ConflictException('ROLE workflow steps require actorRoleId');
    if (dto.steps.some((s) => s.actorType === WorkflowActorType.SPECIFIC_USER && !s.actorUserId)) throw new ConflictException('SPECIFIC_USER workflow steps require actorUserId');
    if (dto.performanceReviewTypeId && !(await this.prisma.performanceReviewType.findFirst({ where: { id: dto.performanceReviewTypeId, program: { organizationId } } }))) throw new NotFoundException('Performance review type not found');
    try {
      return await this.prisma.workflow.create({ data: { organizationId, performanceReviewTypeId: dto.performanceReviewTypeId ?? null, name: dto.name.trim(), code: dto.code.trim().toUpperCase(), description: dto.description?.trim() || null, steps: { create: dto.steps.map((s) => ({ stepOrder: s.stepOrder, name: s.name.trim(), description: s.description?.trim() || null, actorType: s.actorType, actorRoleId: s.actorRoleId ?? null, actorUserId: s.actorUserId ?? null, required: s.required ?? true, canReturn: s.canReturn ?? true, canReject: s.canReject ?? false })) } }, include: { steps: { orderBy: { stepOrder: 'asc' } } } });
    } catch (e) { if ((e as any).code === 'P2002') throw new ConflictException('Workflow code already exists'); throw e; }
  }
  ...
  async submit(organizationId: string, reviewId: string, userId: string, dto: WorkflowActionDto) {
    ...
    const submitStatuses: PerformanceReviewStatus[] = [PerformanceReviewStatus.DRAFT, PerformanceReviewStatus.IN_PROGRESS, PerformanceReviewStatus.RETURNED, PerformanceReviewStatus.RESUBMITTED];
    if (!submitStatuses.includes(review.status)) throw new ConflictException('This review cannot be submitted from its current status');
    await this.validateSubmissionCompleteness(review.id);
  ...
  async act(organizationId: string, reviewId: string, userId: string, action: WorkflowActionType, dto: WorkflowActionDto) {
    ...
    if (action === WorkflowActionType.RETURN && !currentStep.canReturn) throw new ConflictException('This workflow step does not allow return');
    if (action === WorkflowActionType.REJECT && !currentStep.canReject) throw new ConflictException('This workflow step does not allow rejection');
    const actionable: WorkflowActionType[] = [WorkflowActionType.APPROVE, WorkflowActionType.RETURN, WorkflowActionType.REJECT, WorkflowActionType.CANCEL];
    if (!actionable.includes(action)) throw new ConflictException('Unsupported workflow action');
    const steps = instance.workflow.steps; const index = steps.findIndex((s) => s.id === instance.currentStepId); const nextStep = action === WorkflowActionType.APPROVE ? steps[index + 1] : undefined; const now = new Date();
    const completed = action === WorkflowActionType.APPROVE && !nextStep;
    const terminalActions: WorkflowActionType[] = [WorkflowActionType.RETURN, WorkflowActionType.REJECT, WorkflowActionType.CANCEL];
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const result = await tx.workflowInstance.update({ where: { id: instance.id }, data: { status: newStatus, currentStepId: nextStep?.id ?? null, completedAt: completed || terminalActions.includes(action) ? now : null, actions: { create: { workflowStepId: currentStep.id, performedBy: userId, action, comment: dto.comment?.trim() || null } } }, include: { currentStep: true } });
      await tx.performanceReview.update({ where: { id: review.id }, data: { status: reviewStatus, completedAt: completed ? now : undefined } }); return result;
    });
    await this.audit.record({ userId, action: `REVIEW_${action}`, module: 'performance', entityType: 'PerformanceReview', entityId: review.id, newValues: { reviewStatus, workflowStatus: newStatus, currentStepId: nextStep?.id ?? null } });
    if (nextStep) await this.notifyActor(organizationId, review.id, nextStep, userId); return updated;
  }
  async finalize(organizationId: string, reviewId: string, userId: string) {
    const review = await this.prisma.performanceReview.findFirst({ where: { id: reviewId, employee: { organizationId } }, include: { kpis: true, competencies: true, workflowInstance: true, scores: { orderBy: { calculatedAt: 'desc' }, take: 1 } } });
    if (!review) throw new NotFoundException('Performance review not found');
    if (review.status !== PerformanceReviewStatus.APPROVED) throw new ConflictException('Only approved reviews can be finalized');
    if (!review.workflowInstance || review.workflowInstance.status !== WorkflowStatus.COMPLETED) throw new ConflictException('The review workflow must be completed before finalization');
    const incompleteKpis = review.kpis.filter((k) => k.supervisorScore === null && k.employeeScore === null); const incompleteCompetencies = review.competencies.filter((c) => c.supervisorRating === null && c.employeeRating === null);
    if (incompleteKpis.length || incompleteCompetencies.length) throw new ConflictException(`Complete all KPI and competency assessments before finalization (${incompleteKpis.length} KPI${incompleteKpis.length === 1 ? '' : 's'}, ${incompleteCompetencies.length} competenc${incompleteCompetencies.length === 1 ? 'y' : 'ies'} outstanding)`);
    const latestScore = review.scores[0]; if (!latestScore || latestScore.overallScore === null) throw new ConflictException('Calculate the final performance score before finalization');
    const finalized = await this.prisma.performanceReview.update({ where: { id: review.id }, data: { status: PerformanceReviewStatus.FINALIZED, completedAt: review.completedAt ?? new Date() } });
    await this.audit.record({ userId, action: 'REVIEW_FINALIZED', module: 'performance', entityType: 'PerformanceReview', entityId: review.id, newValues: { status: PerformanceReviewStatus.FINALIZED } }); return finalized;
  }
  private async validateSubmissionCompleteness(reviewId: string) {
    const review = await this.prisma.performanceReview.findUnique({ where: { id: reviewId }, include: { kpis: true, competencies: true } }); if (!review) throw new NotFoundException('Performance review not found');
    const incompleteKpis = review.kpis.filter((k) => k.employeeScore === null); const incompleteCompetencies = review.competencies.filter((c) => c.employeeRating === null);
    if (incompleteKpis.length || incompleteCompetencies.length) throw new ConflictException(`Complete all employee assessments before submitting the review (${incompleteKpis.length} KPI${incompleteKpis.length === 1 ? '' : 's'}, ${incompleteCompetencies.length} competenc${incompleteCompetencies.length === 1 ? 'y' : 'ies'} outstanding)`);
    const totalWeight = review.kpis.reduce((sum, k) => sum + Number(k.weight), 0) + review.competencies.reduce((sum, c) => sum + Number(c.weight), 0); if (totalWeight <= 0) throw new ConflictException('A review must contain at least one item with a weight greater than zero');
  }
  private async delegate(organizationId: string, reviewId: string, instance: { id: string; currentStepId: string | null }, userId: string, dto: WorkflowActionDto) {
    if (!dto.delegateToUserId) throw new ConflictException('delegateToUserId is required'); if (dto.delegateToUserId === userId) throw new ConflictException('A user cannot delegate to themselves');
    const target = await this.prisma.user.findFirst({ where: { id: dto.delegateToUserId, employee: { organizationId }, accountStatus: 'ACTIVE' } }); if (!target) throw new NotFoundException('Delegation target not found');
    const now = new Date(); const end = new Date(now); end.setDate(end.getDate() + 30);
    const action = await this.prisma.$transaction(async (tx) => { await tx.delegation.create({ data: { delegatedBy: userId, delegatedTo: dto.delegateToUserId!, workflowId: instance.id, startDate: now, endDate: end, reason: dto.comment?.trim() || null } }); return tx.workflowAction.create({ data: { workflowInstanceId: instance.id, workflowStepId: instance.currentStepId!, performedBy: userId, action: WorkflowActionType.DELEGATE, comment: dto.comment?.trim() || `Delegated to ${target.username}` } }); });
  ...
  private async canAct(stepId: string, reviewId: string, workflowInstanceId: string, userId: string) {
    ...
    if (step.actorType === WorkflowActorType.ROLE || step.actorType === WorkflowActorType.HR || step.actorType === WorkflowActorType.PMS_ADMIN) {
      const user = await this.prisma.user.findUnique({ where: { id: userId }, include: { roles: { include: { role: true } } } }); if (!user) return false;
      ...
      const code = step.actorType === WorkflowActorType.HR ? 'HR_OFFICER' : 'PMS_ADMIN'; return user.roles.some((ur) => ur.role.code === code && (!ur.expiresAt || ur.expiresAt > new Date()));
    }
    return false;
  }
